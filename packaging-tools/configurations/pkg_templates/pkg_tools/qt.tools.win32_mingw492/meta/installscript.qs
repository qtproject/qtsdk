...
function Component()
{
    ...
    // set the default values to MINGW492_DIR
...
Component.prototype.reactOnTargetDirChange = function(key, value)
{
    if (key == "TargetDir") {
        ...
        installer.setValue("MINGW492_DIR", path.replace(/\\/g, "/"));
    }
}
...
Component.prototype.createOperations = function()
{
    ...
    if (installer.value("os") == "win") {
        try {
            ...
            installer.setValue("MINGW492_TCID", tcId);
            installer.setValue("MINGW492_DBGID", dbgId);
            ...
            component.addOperation("Execute",
                                   ["{0,2}", "@SDKToolBinary@", "addDebugger",
                                    "--id", dbgId,
                                    "--name", "GNU gdb 7.8 for MinGW 4.9.2 32bit",
                                    "--binary", "@MINGW492_DIR@/bin/gdb.exe",
            ...
            component.addOperation("Execute",
                                   ["{0,4}", "@SDKToolBinary@", "addKeys",
                                    "android", "MakeExtraSearchDirectory", "QString:@MINGW492_DIR@/bin"]);
        } catch( e ) {
            print( e );
        }
    }
