...
function createShortcuts()
{
    ...
    component.addOperation( "CreateShortcut",
                            cmdLocation,
                            "@StartMenuDir@/5.2.1/MSVC 2010 OpenGL/Qt " + qtStringVersion + " for Desktop (MSVC 2010 OpenGL).lnk",
                            "/A /Q /K " + batchFileName);
    // Assistant
    component.addOperation( "CreateShortcut",
                            component_root_path + "/bin/assistant.exe",
                            "@StartMenuDir@/5.2.1/MSVC 2010 OpenGL/Assistant.lnk");

    // Designer
    component.addOperation( "CreateShortcut",
                            component_root_path + "/bin/designer.exe",
                            "@StartMenuDir@/5.2.1/MSVC 2010 OpenGL/Designer.lnk");

    // Linguist
    component.addOperation( "CreateShortcut",
                            component_root_path + "/bin/linguist.exe",
                            "@StartMenuDir@/5.2.1/MSVC 2010 OpenGL/Linguist.lnk");
}
    ...
    if (installer.value("os") == "win") {
        try {
            ...
            component.addOperation("Execute",
                                   ["@SDKToolBinary@", "addQt",
                                    "--id", component.name,
                                    "--name", "Qt 5.2.1 MSVC2010 32bit OpenGL",
            ...
            component.addOperation("Execute",
                                   ["@SDKToolBinary@", "addKit",
                                    "--id", kitName,
                                    "--name", "Desktop Qt 5.2.1 MSVC2010 32bit OpenGL",
