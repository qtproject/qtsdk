...
def get_component_data(
        ...
        # For payload already in IFW compatible format, use the raw artifact and continue
            ...
            if download_path.suffix != Path(archive.archive_name).suffix:
                log.warning(
                    ...
                    download_path.name, archive.archive_name
                )
            shutil.move(str(download_path), str(Path(data_dir_dest) / archive.archive_name))
            return
        # Extract payload archive when required to be patched or recompressed to compatible format
        else:
            if not extract_file(str(download_path), install_dir):
                # Raise error on unsuccessful extraction
                raise CreateInstallerError(f"Couldn't extract archive: {download_path}")
    ...
    # Lastly, compress the component back to an archive
    ...
    save_as = os.path.normpath(os.path.join(data_dir_dest, archive.archive_name))
    arch_format = Path(archive.archive_name).suffix.strip(".")
    run_cmd(cmd=[task.archivegen_tool, "-f", arch_format, save_as] + content_list, cwd=data_dir_dest)
            ...
            for archive in sdk_comp.downloadable_archives:
                ...
                if task.offline_installer or task.create_repository:
                    ...
                    compress_content_dir = os.path.normpath(
                        temp_data_dir + os.sep + archive.archive_name
                    )
