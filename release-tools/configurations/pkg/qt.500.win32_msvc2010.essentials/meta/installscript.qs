    ...
    if (installer.value("os") == "win") {
        try {
            ...
            component.addOperation("Execute",
                new Array("{1}", "@SDKToolBinary@", "addQt", "--id", component.name, "--name", "Qt %QT_VERSION%-rc2 MSVC2010 32bit (SDK)", "--type", "Qt4ProjectManager.QtVersion.Desktop", "--qmake", qmakeBinary));

            component.addOperation("Execute",
                new Array("{1}", "@SDKToolBinary@", "addKit", "--id", component.name + "_kit", "--name", "Desktop Qt %QT_VERSION%-rc2 MSVC2010 32bit (SDK)", "--toolchain", "x86-windows-msvc2010-pe-32bit", "--qt", component.name, "--debuggerengine", "4", "--devicetype", "Desktop"));
