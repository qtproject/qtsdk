...
function createShortcuts()
{
    ...
    component.addOperation( "CreateShortcut",
                            cmdLocation,
                            "@StartMenuDir@/5.1.0/MSVC 2012 (32-bit)/Qt " + qtStringVersion + " 32-bit for Desktop (MSVC 2012).lnk",
                            "/A /Q /K " + batchFileName);
    // Assistant
    component.addOperation( "CreateShortcut",
                            component_root_path + "/bin/assistant.exe",
                            "@StartMenuDir@/5.1.0/MSVC 2012 (32-bit)/Assistant.lnk");

    // Designer
    component.addOperation( "CreateShortcut",
                            component_root_path + "/bin/designer.exe",
                            "@StartMenuDir@/5.1.0/MSVC 2012 (32-bit)/Designer.lnk");

    // Linguist
    component.addOperation( "CreateShortcut",
                            component_root_path + "/bin/linguist.exe",
                            "@StartMenuDir@/5.1.0/MSVC 2012 (32-bit)/Linguist.lnk");
}
    ...
    if (installer.value("os") == "win") {
        try {
            ...
            component.addOperation("Execute",
                new Array("{0}", "@SDKToolBinary@", "addQt", "--id", component.name, "--name", "Qt 5.1.0 MSVC2012 32bit", "--type", "Qt4ProjectManager.QtVersion.Desktop", "--qmake", qmakeBinary));

            component.addOperation("Execute",
                new Array("{0}", "@SDKToolBinary@", "addKit", "--id", component.name + "_kit", "--name", "Desktop Qt 5.1.0 MSVC2012 32bit", "--toolchain", "x86-windows-msvc2012-pe-32bit", "--qt", component.name, "--debuggerengine", "4", "--devicetype", "Desktop"));
