...
Component.prototype.createOperations = function()
{
    ...
    if (installer.value("os") == "x11") {
        try {
            ...
            if (installer.value("qt.502.gcc_64.essentials_qtpath"))
                component.addOperation("QtPatch", "linux", installer.value("qt.502.gcc_64.essentials_qtpath"));
            if (installer.value("qt.502.gcc.essentials_qtpath"))
                component.addOperation("QtPatch", "linux", installer.value("qt.502.gcc.essentials_qtpath"));
        } catch( e ) {
            print( e );
        }
    }
    ...
    if (installer.value("os") == "win") {
        try {
            ...
            if (installer.value("qt.502.win64_msvc2012_64.essentials_qtpath"))
                component.addOperation("QtPatch", "windows", installer.value("qt.502.win64_msvc2012_64.essentials_qtpath"));
        } catch( e ) {
            print( e );
        }
    }
    ...
    if (installer.value("os") == "mac") {
        try {
            // patch Qt binaries
            component.addOperation("QtPatch", "mac", installer.value("qt.502.clang_64.essentials_qtpath"));
        } catch( e ) {
            print( e );
        }
    }
