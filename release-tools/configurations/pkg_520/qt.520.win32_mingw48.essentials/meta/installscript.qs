...
function createShortcuts()
{
    ...
    component.addOperation( "CreateShortcut",
                            cmdLocation,
                            "@StartMenuDir@/5.2.0/MinGW 4.8 (32-bit)/Qt " + qtStringVersion + " for Desktop (MinGW 4.8 32 bit).lnk",
                            "/A /Q /K " + batchFileName);
    // Assistant
    component.addOperation( "CreateShortcut",
                            component_root_path + "/bin/assistant.exe",
                            "@StartMenuDir@/5.2.0/MinGW 4.8 (32-bit)/Assistant.lnk");

    // Designer
    component.addOperation( "CreateShortcut",
                            component_root_path + "/bin/designer.exe",
                            "@StartMenuDir@/5.2.0/MinGW 4.8 (32-bit)/Designer.lnk");

    // Linguist
    component.addOperation( "CreateShortcut",
                            component_root_path + "/bin/linguist.exe",
                            "@StartMenuDir@/5.2.0/MinGW 4.8 (32-bit)/Linguist.lnk");


    // Examples & Demos
    //component.addOperation( "CreateShortcut",
    //                        component_root_path + "/bin/qtdemo.exe",
    //                        "@StartMenuDir@/5.2.0/MinGW 4.8 (32-bit)/Examples & Demos.lnk");
}
        ...
        try {
            ...
            component.addOperation("Execute",
                                   ["@SDKToolBinary@", "addKit",
                                    "--id", kitName,
                                    "--name", "Desktop Qt 5.2.0 MinGW 32bit",
                                    "--toolchain", tcId,
                                    "--qt", component.name,
                                    "--debuggerengine", "1",
