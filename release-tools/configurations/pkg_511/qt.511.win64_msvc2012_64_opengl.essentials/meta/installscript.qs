...
function Component()
{
    ...
    if ( installer.environmentVariable("ProgramFiles(x86)") == "" ) {
        installer.componentByName("qt.511.win64_msvc2012_64_opengl").setValue("Virtual", "true")
        installer.componentByName("qt.511.win64_msvc2012_64_opengl").setValue("Default", "false")
        installer.componentByName("qt.511.win64_msvc2012_64_opengl.essentials").setValue("Virtual", "true")
        installer.componentByName("qt.511.win64_msvc2012_64_opengl.essentials").setValue("Default", "false")
        installer.componentByName("qt.511.win64_msvc2012_64_opengl.addons").setValue("Virtual", "true")
        installer.componentByName("qt.511.win64_msvc2012_64_opengl.addons").setValue("Default", "false")
    } else {
        // all ok
    }
}
...
function createShortcuts()
{
    ...
    component.addOperation( "CreateShortcut",
                            cmdLocation,
                            "@StartMenuDir@/5.1.1/MSVC 2012 OpenGL (64-bit)/Qt " + qtStringVersion + " 64-bit for Desktop (MSVC 2012 OpenGL).lnk",
                            "/A /Q /K " + batchFileName);
    // Assistant
    component.addOperation( "CreateShortcut",
                            component_root_path + "/bin/assistant.exe",
                            "@StartMenuDir@/5.1.1/MSVC 2012 OpenGL (64-bit)/Assistant.lnk");

    // Designer
    component.addOperation( "CreateShortcut",
                            component_root_path + "/bin/designer.exe",
                            "@StartMenuDir@/5.1.1/MSVC 2012 OpenGL (64-bit)/Designer.lnk");

    // Linguist
    component.addOperation( "CreateShortcut",
                            component_root_path + "/bin/linguist.exe",
                            "@StartMenuDir@/5.1.1/MSVC 2012 OpenGL (64-bit)/Linguist.lnk");
}
    ...
    if (installer.value("os") == "win") {
        try {
            ...
            component.addOperation("Execute",
                new Array("{0}", "@SDKToolBinary@", "addQt", "--id", component.name, "--name", "Qt 5.1.1 MSVC2012 OpenGL 64bit", "--type", "Qt4ProjectManager.QtVersion.Desktop", "--qmake", qmakeBinary));

            component.addOperation("Execute",
                new Array("{0}", "@SDKToolBinary@", "addKit", "--id", component.name + "_kit", "--name", "Desktop Qt 5.1.1 MSVC2012 OpenGL 64bit", "--toolchain", "x86-windows-msvc2012-pe-64bit", "--qt", component.name, "--debuggerengine", "4", "--devicetype", "Desktop"));
